...
/**为什么要做这个工具软件?*/
- (void)doProgrammer{
    NSArray *details=@[@"公司的项目界面太多,为了防止后面界面需求修改,不得不基本上写成tableView界面,写多了才发现好烦,一个不是很难的tableView界面(大概就三种自定义Cell)至少写上半小时,困死人",
                       @"写了十几个界面返回来一看,好像自己像机器人一样,重复着创建文件,导入文件,重写代理方法,model属性,控件赋值等等,就决定做个模板,才写了个 \"快速生成代码(tableView,collectionView)\"",
                       @"做完发现,自己搭界面的速度提高好多倍,最重要的是,不用去做太多的重复劳动,因为这些劳动量还是要我们去思考一下(虽然不难),我们做这行本身就是高强度脑力工作,要写很多逻辑思维代码,如果还要花时间去写这些无意义的代码,既无聊犯困,还会降低我们热情和动力去实现页面中更加重要的核心逻辑设计",
                       @"就这样,渐渐尝到了甜头,因为下班比以前早了,而且再次修改代码时发现都是一个模子里刻出来的,很有规律,比较好改",
                       @"尝到甜头后任然觉得还是有很多重复劳动,意识到电脑本身就是帮我们加快工作效率的,这样才渐渐有了其它新的功能",
                       @"虽然算不上什么好软件,但是觉得能为大家减少工作量(因为干过咋们这行的才知道,码代码的不易),我还是厚着脸皮上传上去",
                       @"做了一段时间,发现工程越来越大,功能越来越多,因为项目框架没设计好,就导致了很多朋友本来想看看怎么写的,发现代码写的很乱.在这里表示:不好意思,希望大家仅仅把它当做工具用",
                       @"也有些朋友觉得很棒很厉害,其实是大家没自己去写,写了才知道,原来就是字符串操作(基本上就这点原理),写庞大了看上去觉得厉害而已",
                       @"当然比起其它开源项目来说,这点功能有点相形见绌了,就着这个项目工具的目的,还是希望能帮助部分朋友",
                       @"后期会继续添加新的功能,希望有兴趣的小伙伴关注!(俯首甘为孺子牛,祝大家生活和工作开开心心!)"
                       ];
...
- (void)loadData{
    ...
    }else if ([_helpString isEqualToString:@"为什么要做这个工具软件?"]){
        [self doProgrammer];
    }
